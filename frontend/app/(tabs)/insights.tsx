    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.emptyText}>Unable to load health scores</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.subtitle}>Property performance at a glance</Text>
        </View>
        <View style={[styles.headerBadge, { backgroundColor: getScoreBgColor(data.portfolio_average) }]}>
          <Ionicons
            name={getStatusIcon(data.portfolio_status) as any}
            size={22}
            color={getScoreColor(data.portfolio_average)}
          />
        </View>
      </View>
      ...
      >
        ...
        <Card style={styles.portfolioCard}>
          <View style={styles.portfolioContent}>
            ...
            <View style={styles.portfolioInfo}>
              <Text style={styles.portfolioLabel}>Portfolio Average</Text>
              <View style={[
                styles.statusBadge,
                { backgroundColor: getScoreBgColor(data.portfolio_average) }
              ]}>
                <Ionicons
                  name={getStatusIcon(data.portfolio_status) as any}
                  size={14}
              ...
              <Text style={styles.portfolioDesc}>
                Based on {data.properties.length} propert{data.properties.length === 1 ? 'y' : 'ies'}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.legend}>
            <View style={styles.legendItem}>
              ...
              <Text style={styles.legendText}>70-100 Healthy</Text>
            </View>
            <View style={styles.legendItem}>
              <View style={[styles.legendDot, { backgroundColor: theme.colors.warning }]} />
              <Text style={styles.legendText}>40-69 Moderate</Text>
            </View>
            <View style={styles.legendItem}>
              <View style={[styles.legendDot, { backgroundColor: theme.colors.error }]} />
              <Text style={styles.legendText}>0-39 At Risk</Text>
            </View>
          </View>
        </Card>
        ...
        {data.properties.length === 0 ? (
          <Card style={styles.emptyCard}>
            ...
            <Text style={styles.emptyCardText}>Add properties to see their health scores</Text>
          </Card>
        ) : (
        ...
        <Card style={styles.explainerCard}>
          <View style={styles.explainerHeader}>
            ...
            <Text style={styles.explainerTitle}>How Scores Work</Text>
          </View>
          <View style={styles.explainerGrid}>
            <View style={styles.explainerItem}>
              <Text style={styles.explainerLabel}>Rent Collection</Text>
              <Text style={styles.explainerValue}>30 pts</Text>
            </View>
            <View style={styles.explainerItem}>
              <Text style={styles.explainerLabel}>Occupancy</Text>
              <Text style={styles.explainerValue}>25 pts</Text>
            </View>
            <View style={styles.explainerItem}>
              <Text style={styles.explainerLabel}>Maintenance</Text>
              <Text style={styles.explainerValue}>20 pts</Text>
            </View>
            <View style={styles.explainerItem}>
              <Text style={styles.explainerLabel}>Lease Stability</Text>
              <Text style={styles.explainerValue}>25 pts</Text>
            </View>
          </View>
        </Card>
      ...
      <TouchableOpacity onPress={onToggle} activeOpacity={0.7} style={styles.propertyTouchable}>
        ...
        <View style={styles.quickStats}>
          ...
          <View style={styles.quickStatItem}>
            ...
            <Text style={styles.quickStatText}>
              {Math.round(property.collection_rate)}% collected
            </Text>
          </View>
          {property.open_issues > 0 && (
            <View style={styles.quickStatItem}>
              ...
              <Text style={[styles.quickStatText, { color: theme.colors.warning }]}>
                {property.open_issues} issue{property.open_issues > 1 ? 's' : ''}
              </Text>
            </View>
          )}
          {property.days_to_nearest_expiry !== null && property.days_to_nearest_expiry <= 60 && (
            <View style={styles.quickStatItem}>
              <Ionicons
                name="time-outline"
                size={14}
                color={property.days_to_nearest_expiry <= 0 ? theme.colors.error : property.days_to_nearest_expiry <= 30 ? theme.colors.error : theme.colors.warning}
              />
              <Text style={[
                styles.quickStatText,
                { color: property.days_to_nearest_expiry <= 0 ? theme.colors.error : property.days_to_nearest_expiry <= 30 ? theme.colors.error : theme.colors.warning }
              ]}>
                {property.days_to_nearest_expiry <= 0 ? 'Lease expired' : `${property.days_to_nearest_expiry}d lease`}
              </Text>
            </View>
          )}
        </View>
      ...
      {expanded && (
        <View style={styles.breakdownContainer}>
          ...
          <BreakdownBar
            ...
            value={property.breakdown.rent_collection}
            maxValue={30}
            color={property.breakdown.rent_collection >= 21 ? theme.colors.success : property.breakdown.rent_collection >= 12 ? theme.colors.warning : theme.colors.error}
          />
          <BreakdownBar
            label="Occupancy"
            value={property.breakdown.occupancy}
            maxValue={25}
            color={property.breakdown.occupancy >= 17.5 ? theme.colors.success : property.breakdown.occupancy >= 10 ? theme.colors.warning : theme.colors.error}
          />
          <BreakdownBar
            label="Maintenance Health"
            value={property.breakdown.maintenance}
            maxValue={20}
            color={property.breakdown.maintenance >= 14 ? theme.colors.success : property.breakdown.maintenance >= 8 ? theme.colors.warning : theme.colors.error}
          />
          <BreakdownBar
            label="Lease Stability"
            value={property.breakdown.lease_stability}
            maxValue={25}
            color={property.breakdown.lease_stability >= 17.5 ? theme.colors.success : property.breakdown.lease_stability >= 10 ? theme.colors.warning : theme.colors.error}
          />
...
// Tip renderer based on weakest area
const renderTip = (property: PropertyHealthScore) => {
  const { breakdown } = property;
  const areas = [
    { name: 'Rent Collection', value: breakdown.rent_collection / 30, type: 'rent' },
    { name: 'Occupancy', value: breakdown.occupancy / 25, type: 'occupancy' },
    { name: 'Maintenance', value: breakdown.maintenance / 20, type: 'maintenance' },
    { name: 'Lease Stability', value: breakdown.lease_stability / 25, type: 'lease' },
  ];

  const weakest = areas.reduce((min, a) => (a.value < min.value ? a : min), areas[0]);

  if (weakest.value >= 0.7) return null; // All areas are fine

  let tipText = '';
  let tipIcon: string = 'bulb-outline';

  switch (weakest.type) {
    case 'rent':
      tipText = 'Follow up with tenants who have outstanding rent payments to improve this score.';
      tipIcon = 'cash-outline';
      break;
    case 'occupancy':
      tipText = 'Consider listing vacant units or reviewing rental pricing to attract tenants.';
      tipIcon = 'people-outline';
      break;
    case 'maintenance':
      tipText = 'Resolve open maintenance issues, especially high-priority ones, to boost this score.';
      tipIcon = 'construct-outline';
      break;
    case 'lease':
      tipText = 'Start renewal conversations for expiring leases to maintain stability.';
      tipIcon = 'document-text-outline';
      break;
  }
