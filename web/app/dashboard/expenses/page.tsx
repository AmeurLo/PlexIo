...
import { formatCurrency, formatDate, downloadCsv, daysUntil } from "@/lib/format";
        ...
        <>
          ...
          {tab === "expenses" && (
            <div className="space-y-5">
              ...
              <div className="flex gap-2 flex-wrap">
                <button onClick={() => setCatFilter("all")} className={`px-3.5 py-1.5 rounded-full text-[12px] font-medium transition-all ${catFilter === "all" ? "bg-teal-600 text-white" : "bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-600 hover:border-teal-400"}`}>
                  {lang === "fr" ? "Toutes" : "All"}
                </button>
                {EXPENSE_CATEGORIES.map(c => (
                  <button key={c.value} onClick={() => setCatFilter(c.value)} className={`px-3.5 py-1.5 rounded-full text-[12px] font-medium transition-all ${catFilter === c.value ? "bg-teal-600 text-white" : "bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-600 hover:border-teal-400"}`}>
                    {lang === "fr" ? c.fr : c.en}
                  </button>
                ))}
              </div>
              {filteredExp.length === 0 ? (
                <EmptyState icon="dollar" title={lang === "fr" ? "Aucune dépense" : "No expenses yet"} description={lang === "fr" ? "Ajoutez votre première dépense." : "Add your first expense."} actionLabel={`+ ${lang === "fr" ? "Ajouter" : "Add"}`} onAction={openAddExpense} />
              ) : (
                          ...
                          <tr key={ex.id ?? (ex as any)._id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                            ...
                            <td className="px-5 py-3">
                              <div className="flex gap-2 justify-end">
                                <button onClick={() => openEditExpense(ex)} className="text-[12px] text-teal-700 hover:underline">{lang === "fr" ? "Modifier" : "Edit"}</button>
                                <button onClick={() => setDeleteExpense(ex)} className="text-[12px] text-red-500 hover:underline">{lang === "fr" ? "Supprimer" : "Delete"}</button>
                              </div>
                            </td>
                          </tr>
                        ))}
                      ...
                      <div key={ex.id ?? (ex as any)._id} className="p-4 flex items-center gap-3">
                        ...
                        <div className="flex flex-col items-end gap-1">
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => openEditExpense(ex)} className="text-[11px] text-teal-700 hover:underline">{lang === "fr" ? "Modifier" : "Edit"}</button>
                            <button onClick={() => setDeleteExpense(ex)} className="text-[11px] text-red-500 hover:underline">{lang === "fr" ? "Supprimer" : "Delete"}</button>
                          </div>
                        </div>
                      </div>
                    ))}
          ...
          {tab === "mortgage" && (
            <div className="space-y-5">
              ...
              {mortgages.length === 0 ? (
                <EmptyState icon="home" title={lang === "fr" ? "Aucune hypothèque" : "No mortgages yet"} description={lang === "fr" ? "Ajoutez votre premier prêt." : "Add your first mortgage."} actionLabel={`+ ${lang === "fr" ? "Ajouter" : "Add"}`} onAction={openAddMortgage} />
              ) : (
          ...
          {tab === "insurance" && (
            <div className="space-y-5">
              ...
              {policies.length === 0 ? (
                <EmptyState icon="shield" title={lang === "fr" ? "Aucune assurance" : "No insurance policies"} description={lang === "fr" ? "Ajoutez votre première police." : "Add your first policy."} actionLabel={`+ ${lang === "fr" ? "Ajouter" : "Add"}`} onAction={openAddInsurance} />
              ) : (
        ...
        {tab === "insurance" && (
          <div className="space-y-4">
            ...
            <FormField label={lang === "fr" ? "Tél. assureur" : "Insurer phone"}>
              <input className={inputClass} value={insForm.contact_phone} onChange={e => setInsForm(p => ({...p, contact_phone: e.target.value}))} placeholder="1-800-123-4567" />
            </FormField>
          </div>
        )}
      </Modal>
